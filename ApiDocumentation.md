...
This document outlines the API endpoints for the Sharetribe integration used in the FlutterFlow application. The APIs manage user authentication, profile management, and image uploads, leveraging the Sharetribe SDK. All endpoints are secured and require proper authentication where specified.
Base URL
...
For /login and /create-user:Content-Type: application/json


For all other endpoints:Content-Type: application/json
...
Endpoints using tokenRefreshMiddleware include new_refresh_token and expires_in in responses if the token is refreshed.
Clients must update the stored token with new_refresh_token when provided to maintain session continuity.
...
Common errors:
400: Validation, malformed input, or missing X-Refresh-Token.
